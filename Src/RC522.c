...
char PcdReset(void)
{
    ...
    HAL_Delay(60);  //_NOP();_NOP();
    RC522_RESET_SET();     //RST522_1;RST522_1;
    HAL_Delay(1);  //_NOP();_NOP();
...
unsigned char ReadRawRC(unsigned char Address)
{
	...
	HAL_Delay(1);
	RC522_ENABLE;
	while(HAL_OK!=HAL_SPI_Transmit(&hspi1,&ucAddr,1,0xffff));
	while(HAL_OK!=HAL_SPI_Receive(&hspi1,&ucResult,1,0xffff));
...
{  
	...
	HAL_Delay(1);
	RC522_ENABLE;
//	while(HAL_OK!=HAL_SPI_Transmit(&hspi1,&write_buffer[0],1,0xffff));
//	while(HAL_OK!=HAL_SPI_Transmit(&hspi1,&write_buffer[1],1,0xffff));
	while(HAL_OK!=HAL_SPI_Transmit(&hspi1,write_buffer,2,0xffff));
	RC522_DISABLE;
}
